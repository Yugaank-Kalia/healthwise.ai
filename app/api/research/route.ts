...
import { messages, researchSearchQueries } from '@/src/db/schemas/nutrition-schema';
import {
	answerStream,
	type OrchestratorStreamResult,
} from '@/lib/(research)/orchestrator';
import { extractCitations } from '@/lib/(research)/llm';
...
export async function POST(req: NextRequest) {
	...
	const [{ value: queryCount }] = await db
		.select({ value: count() })
		.from(researchSearchQueries)
		.where(
			and(
				eq(researchSearchQueries.userId, session.user.id),
				gte(researchSearchQueries.createdAt, startOfDay),
			),
		);
